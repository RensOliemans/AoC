...
#+RESULTS[d4e6474f0ecd639a6ae9101d43597f5993d33020]:
: 12839601725877
:
...
#+RESULTS[cbd4c8f8dfc0f1c816079b5a59ee09f322c0b71b]:
: 371
: 
...
But, since it's easily fast enough (runs basically instantly), I don't really care. [[*Day 2][Personal growth!]]
...
#+RESULTS[94c52d7d77683a56a9ca48abc343b43f16930af8]:
: 1229
: 
...
Each position in the above map is given by a number and represents the /height/ of the map. A /hiking trail/ is a path that starts at height =0=, ends at height =9=, always increases by a height of exactly 1 at each step, and don't include diagonal steps. A /trailhead/ is any position that starts one or more hiking trails (so they always start at =0= here). A trailhead's /score/ is the number of 9-height positions reachable from that trailhead.
...
#+begin_src clojure +n 1 :results silent
  (defn trailhead-score [grid trailhead])

  (defn p1 [input]
    (let [grid (aoc.util/adjacent-num-list-per-line input)
          trailheads (aoc.util/char-locations grid 0)]
      (->> trailheads
           (map (partial trailhead-routes grid))
           (map distinct)
           (map count)
           (reduce +))))
#+end_src

=trailhead-routes= is a recursive function, where we find the 9s we can reach, duplicated if they are reachable via multiple paths (spoilers for part 2). We traverse the =position='s neighbours and filter the ones that are 1 step higher. From these neighbours, we call the function again. When we are at height 8, we don't traverse further but instead return the amount of 9's next to us.
...
#+RESULTS[8debbe9a28176d7242cb0ec00207a309ad3088d8]:
: 482
: 

** Part 2
Foreshadowing is complete! We need to find out many distinct routes begin at a given position, and sum that for each of the starting positions. For the example, the sum is *81*. This means that we just remove the ~(map distinct)~ call and we are done.
...
#+RESULTS[96b1e6630ed52301ad108517ed33ef4c5fa4af27]:
: 1094
: 
...
- An engraged number with an even amount of digits will be split into two stones;
- Otherwise, an engraved =n= will be converted to =n \cdot 2024=.

After 25 blinks, the two numbers below will have be converted into *55312* stones. This leads itself nicely to recursion, and even though it grows exponentially that way, I'll have to look into caching/memoization for clojure.
...
#+begin_src clojure +n 1 :results silent
  ...
  (defn p1
    ([input] (p1 input 25))
    ([input blinks]
     (let [numbers (clojure.string/split input #"\s")]
       (->> numbers
            (map read-string)
...
=num-stones= is a recursive function that's pretty straightforward:
#+begin_src clojure +n 1 :results silent
...
#+begin_src clojure +n 1 :results pp :var example=testinput11
