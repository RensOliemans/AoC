...
* Utils
:PROPERTIES:
:header-args: :tangle util.clj :exports both
...
My strategy is: convert the input to pairs of numbers (our util functions take care of this), tranpose them (so we have two lists), sort them, tranpose them again (pairs), and take the difference and sum it.
...
#+begin_src clojure
  (defn p1 [input]
    (let [input (util/string-as-list-of-numbers-per-line input)]
      (->> input
           (transpose)
           (map sort)
           (transpose)
...
#+begin_src clojure :tangle no
  (frequencies (last (transpose (util/string-as-list-of-numbers-per-line testinput))))
#+end_src
...
#+begin_src clojure
  (defn p2 [input]
    (let [input (transpose (util/string-as-list-of-numbers-per-line input))
          one (first input)
...
test it out:
#+begin_src clojure
  (diffs (last (util/string-as-list-of-numbers-per-line testinput)))
#+end_src
...
#+begin_src clojure
  ...
  (defn p1 [input]
    (->> (util/string-as-list-of-numbers-per-line input)
         (filter is-safe?)
         (count)))
