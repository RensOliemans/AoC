...
* Day 3
:PROPERTIES:
:header-args: :tangle day3.clj :exports both
...
This contains the substrings =don't()= and =do()=, which disable and enable =mul()= instructions. I can do fancy clojure things, but Emacs is way too good for this, so let's do it quickly in Elisp. The goal is to simply remove everything from the input file thats in between a =don't()= and a =do()= instruction, and then call =(p1)= on this input.
