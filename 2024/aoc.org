...
* Day 5
:PROPERTIES:
:header-args: :tangle src/day5.clj :exports both
...
#+end_src
Now we can write =p1=. Since I expect to need the orderings, updates and dependency-graph, I'll create a small function =parse-input= that extracts these from the puzzle input.
...
Very reasonable, but we haven't yet filled in =build-dependency-graph= and =sort=. =build-dependency-graph= should return a map of the following form:
#+begin_example
...
Let's test it out on the example input. Verifying that it's correct is left as an exercise for the reader.
#+begin_src clojure :results output :tangle no :var example=testinput5
  (let [[orderings _ _] (parse-input example)
        dependency-graph (build-dependency-graph orderings)]
    (clojure.pprint/pprint dependency-graph))
#+end_src
...
And now, ladies and gentleman, the moment you've all been waiting for, =sort=! We need to sort an =update= based on a =dependency-graph=. =sort= does a couple of things:
1. Create =graph=: This is a subset of =dep-graph=, /limited to the items in update/. It
   starts with an empty map ={}=, and then for each =item= in =update=, gets the elements in
   the =dependency-graph= that depend on =item=. =graph= ends up as a map with key a number,
   and value a set of the dependencies.

   Limiting the dependency graph to be relevant only to the current =update= gives us a
    ...
    (let [graph (reduce (fn [acc item]
      ...
      (vec (sort-by (fn [item]
                      (let [deps (get dep-graph item [])]
                        (count (filter #(contains? (set update) %) deps))))
                    update))))
#+end_src
...
yes
** Part 2
Surprise surprise, we need to sort the incorrect updates; then take the sum of the middle number (of only the /incorrect/ updates!). Our prescience is immeasurable.
#+begin_src clojure +n 1 :results silent
  (defn p2 [input]
