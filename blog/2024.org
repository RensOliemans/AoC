...
** Part 1
We effectively reduce twice: once to reduce all numbers and once to reduce all possible operators. I do the last one in a =mapcat=, but same stuff, more or less. =is-correct?= returns true if the test result can be obtained by any of the input numbers.
...
** Part 2
We introduce a third operator: concatenation. Part two is effectively identical, but for the third operator:
...
This one takes quite long: nearly three seconds. That's not too nice.
